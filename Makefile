...
DOCKER_TAG?=jaytwo_fluentunits_length
...
build: restore
	dotnet build ./jaytwo.FluentUnits.Length.sln

test: unit-test
  
unit-test: build
	rm -rf out/testResults
	dotnet test ./test/jaytwo.FluentUnits.Length.Tests \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.FluentUnits.Length.Tests.trx"

pack:
	rm -rf out/packed
	cd ./src/jaytwo.FluentUnits.Length; \
